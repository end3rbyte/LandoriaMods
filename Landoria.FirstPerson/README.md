...
- Hides the local character, armor, weapons, and equipment while in first person.
...
Run `fov <degrees>` to change the field of view shared by first person, third person, and free fly. Run `fov reset` to restore the default value of 65, or `fov` without a value to report the current field of view. The selected value is saved in the same local configuration.
