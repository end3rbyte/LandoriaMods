...
public sealed class CreatureProtectionPolicyTests
{
    ...
    {
        Assert.True(CreatureProtectionPolicy.CanTarget(true, true, Activity(1, 1)));
    }

    [Fact]
    public void MonsterIgnoresBuildingWhenCreatorIsDisconnected()
    {
        Assert.False(CreatureProtectionPolicy.CanTarget(true, true, Activity(1, 2)));
    }

    [Fact]
    public void VanillaIneligibleTargetRemainsIgnored()
    {
        Assert.False(CreatureProtectionPolicy.CanTarget(true, false, 1f));
    }

    [Fact]
    public void DisabledProtectionKeepsVanillaTargeting()
    {
        Assert.True(CreatureProtectionPolicy.CanTarget(false, true, Activity(1, 2)));
    }
