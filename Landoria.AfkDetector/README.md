...
- Requests and confirms a final CharacterVault save before disconnecting the player.
- Logs the correlated CharacterVault request and revision around each disconnect.
...
Install matching AfkDetector and CharacterVault versions on the server and every client.
The client component receives the disconnect reason and CharacterVault performs
the final save handshake. Detection, configuration, and the decision to
disconnect remain server-authoritative. If the save cannot be confirmed, the
player is not disconnected and inactivity monitoring resumes.
